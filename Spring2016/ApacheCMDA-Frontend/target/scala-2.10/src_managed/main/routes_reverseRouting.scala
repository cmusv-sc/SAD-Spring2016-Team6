...
// @LINE:76
class ReverseIteratorPatternDemo {
    

// @LINE:76
def main(wid:Long): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "workflow/nextpage/" + implicitly[PathBindable[Long]].unbind("wid", wid))
}
...
def main : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.StatePatternDemo.main",
   """
      function(offset) {
...
// @LINE:76
def main : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.IteratorPatternDemo.main",
   """
      function(wid) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "workflow/nextpage/" + (""" + implicitly[PathBindable[Long]].javascriptUnbind + """)("wid", wid)})
      }
   """
)
...
def main(offset:Long): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.StatePatternDemo.main(offset), HandlerDef(this, "controllers.StatePatternDemo", "main", Seq(classOf[Long]), "GET", """""", _prefix + """timeline/$offset<[^/]+>""")
)
...
// @LINE:76
def main(wid:Long): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.IteratorPatternDemo.main(wid), HandlerDef(this, "controllers.IteratorPatternDemo", "main", Seq(classOf[Long]), "GET", """""", _prefix + """workflow/nextpage/$wid<[^/]+>""")
)
