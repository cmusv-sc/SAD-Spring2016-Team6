...
public class WorkflowController extends Controller {
    ...
    public static Result parseXML(Long id){
        ObjectNode jnode = importFilter(id);
        if (jnode == null) {
            return redirect(routes.WorkflowController.main());
        }
        JsonNode wfresponse = Workflow.create(jnode);
    ...
    public static ObjectNode importFilter(Long id) {
        ...
        String wfTags = "";
        String wfCode = "";
        String wfImgPath = "/public/images/service.jpeg";
        try {
            ...
            for (int i=0; i<artList.getLength(); i++)
            {
                ...
                if(oneArticle instanceof Element){
                    switch(oneArticle.getNodeName()){
                        ...
                        case "description":wfDesc = (oneArticle.getTextContent()); break;
                        case "content-uri": wfCode = (oneArticle.getTextContent()); break;
                        case "preview":
                            wfImgPath = (oneArticle.getTextContent()); break;
                        case "tags":
        ...
        wfImgPath = wfImgPath.replaceFirst("public", "assets");
        try {
            jnode.put("userID", session("id"));
            jnode.put("wfTitle", wfTitle);
            jnode.put("wfCategory", "MyExperiment");
            jnode.put("wfCode", wfCode);
            jnode.put("wfDesc", Jsoup.parse(wfDesc).text());
            jnode.put("wfGroupId", 0);
            jnode.put("wfImg", wfImgPath);
            jnode.put("wfInput", "Please Edit This Field");
            jnode.put("wfUrl", "http://www.myexperiment.org/workflow.xml?id=" + id.toString());
            jnode.put("wfOutput", "Please Edit This Field");
            jnode.put("wfTags", wfTags);
        }catch(Exception e) {
            flash("error", "Form value invalid");
        }
        return jnode;
